...
1.1. Программа обращается к методу read_report родительского класса REPORT.
...
### работа кода class WarehauseAnalyze
...
3. 3.  Обращаемся к методу . init() класса WarehauseAnalyze
- Вызывает init() родительского класса, передавая ему значения аргументов - '', количество колонок qty_column - 7 и name_of_column название самих колонок - ['Warehouse ID','Qty SKU','Sale','Dispose',  'Max Cell_ID', 'First arrival']. 
3.2. Создаётся атрибут класса WarehauseAnalyze -  sku_rows, в который передается список объектов класса SKU.

4.  Создаем объект Warehouse_report
# Нужно узнать какой параметр во времени нам необходим

5.  Метод _analyze_warehouse():
# поиск данных за весь период времени
- создаем пустой словарь, в котором будем хранить количество необходимых операций для каждого 'Warehouse ID'
- если 'Warehouse ID' не встречалось раньше, создаем новый список операций
- добавляем в найденную операцию для соответствующего 'Warehouse ID' в список
- объявляем новые переменные в которых будем заполнять количество необходимых нам операций: total_qty, total_sale, total_dispose, max_qty(максимальная нагрузка склада из расчёта времени за весь период отображаются в total_qty ),  first_arrival .
- для каждого Warehouse ID ищем общее количество продаж, если вид операции sale, добавляем +1 в переменную total_sale
- для каждого Warehouse ID ищем общее количество утилизаций, если вид операции dispose, добавляем +1 в переменную total_dispose
- для каждого Warehouse ID ищем общее количество товара находящийся на определенном складе, если вид операции не sale и не dispose, добавляем +1 в переменную total_ qty
- для каждого Warehouse ID ищем самое раннее поступление товара на каждый склад, если вид операции first_arrival и добавляем в переменную first_arrival
- …
